    ...
    <div class="py-4">
        <div class="dropdown py-3">
            ...
            <div class="dropdown-menu dashboard-dropdown dropdown-menu-start mt-2 py-1">
                <a class="dropdown-item d-flex align-items-center" href="{{ route('admin.scheduled-posts.create') }}">
                            ...
                            @if (count($posts) > 0)
                                @foreach ($posts as $post)
                                    <tr>
                                        ...
                                        <td>{{ $post->status }}</td>
                                        <td>
                                            @if ($post->fbPages != null)
                                                @foreach ($post->fbPages as $page)
                                                    <span class="btn btn-primary fs-10">{{ $page->name }}</span>
                                                @endforeach
                                            @endif
                                        </td>
                                        ...
                                        <td class="d-flex gap-3">
                                            <a href="{{ route('admin.scheduled-posts.edit', $post->id) }}"
                                                class="btn btn-primary">Edit</a>
                                            <form action="{{ route('admin.scheduled-posts.destroy', $post->id) }}"
